...
Unfortunately the event rule could not be created because of the following error message:

```
User: arn:aws:iam::058258612171:user/hugo.huart is not authorized to perform: scheduler:CreateSchedule on resource: arn:aws:scheduler:us-east-1:058258612171:schedule/default/MeteoswissIngestGrHFerchichiHuart because no identity-based policy allows the scheduler:CreateSchedule action
```

A role with a single policy permitting Lambda functions invocation was created for it.
